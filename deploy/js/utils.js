...
window.Utils = (function() {
  ...
  function inlineConfirm(container, message, onConfirm, options) {
    ...
    bar.id = barId;
    bar.className = 'inline-confirm-bar' + (options.danger ? ' danger' : '');
    bar.innerHTML = '<span class="confirm-msg">' + escapeHtml(message) + '</span>' +
      '<button class="btn btn-sm btn-secondary" data-confirm-cancel>No</button>' +
      '<button class="btn btn-sm ' + (options.danger ? 'btn-danger' : 'btn-warning') + '" data-confirm-ok>Yes</button>';
    // Insert at top of container or before a target element
    var target = options.target || container.firstChild;
    if (target && target.parentNode === container) {
      container.insertBefore(bar, target);
    } else {
